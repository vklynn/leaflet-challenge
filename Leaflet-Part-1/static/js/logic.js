...
d3.json(queryURL).then(function(data){
    // get response, send the data.features and data.features object to the createFeatures function.
...
function createFeatures(earthquakeData, platesData){
    ...
    // Send earthquakes layer to the createMap function & start creating the map and add features
    createMap(earthquakes);
}

// Circles color palette based on magitude (feature) data marker: data markers should reflect the magnitude of the earthquake by their size and the depth of the earthquake by color. Earthquakes with higher magnitudes should appear larger, and earthquakes with greater depth should appear darker in color.
...
// Create map legend for map data
...
legend.onAdd = function() {
    ...
    // go through each magnitude item to label & color the legend
...
function createMap(earthquakes) {
   // Define outdoors & graymap layers
  ...
  // Create overlay object to hold earthquake data
  let overlayMaps = {
    Earthquakes: earthquakes
  };

  // Create our map, using streetmap and earthquakes layers to display on load
  let myMap = L.map("map", {
    center: [
      39.8282, -98.5795
    ],
